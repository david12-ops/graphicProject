...
import com.example.viewUtils.LineRasterizerGraphics;
import com.example.viewUtils.RasterBufferedImage;
        ...
        panel.addComponentListener(new ComponentAdapter() {
            ...
            public void componentResized(ComponentEvent e) {
                if (panel.getWidth() < 1 || panel.getHeight() < 1)
                    return;
                if (panel.getWidth() <= raster.getWidth() && panel.getHeight() <= raster.getHeight()) // no resize if
                                                                                                      // new one is
                                                                                                      // smaller
                    return;
